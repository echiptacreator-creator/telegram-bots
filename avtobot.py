...
from subscription_utils import ensure_user, has_access, PRICE
from aiogram import Bot
from stats_utils import add_post_stat, add_group_stat
from subscription_watcher import subscription_watcher
from subscription_utils import load_subs
from payment_utils import load_payments
import time
...
SAVED_GROUPS_FILE = os.path.join(BASE_DIR, "saved_groups.json")

def save_group_safely(user_id: int, dialog, username: str | None = None):
    data = {}

    if os.path.exists(SAVED_GROUPS_FILE):
        try:
            with open(SAVED_GROUPS_FILE, "r", encoding="utf-8") as f:
                content = f.read().strip()
                if content:
                    data = json.loads(content)
        except Exception as e:
            print("⚠️ saved_groups.json o‘qishda xato:", e)
            data = {}
    ...
    with open(SAVED_GROUPS_FILE, "w", encoding="utf-8") as f:
...
def load_saved_groups():
    if not os.path.exists(SAVED_GROUPS_FILE):
        return []
    ...
    groups = []
    for user_groups in data.values():
    ...
    try:
        ...
        try:
            with open(SAVED_GROUPS_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
                for user_groups in data.values():
                    if str(group_id) in user_groups:
                        saved_by = user_groups[str(group_id)].get("saved_by")
                        break
        except Exception:
            pass

        if saved_by:
            username = user_groups[str(group_id)].get("saved_by_username")

            if username:
                contact = f"@{username}"
            else:
                contact = f"Telegram ID: {saved_by} (username yo‘q)"
